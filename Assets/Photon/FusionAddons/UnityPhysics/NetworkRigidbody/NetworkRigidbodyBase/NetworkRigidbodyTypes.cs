...
namespace Fusion.UnityPhysics {
  ...
  // Internal enum used for caching Authority (to avoid relentlessly calling the property)
  [Flags]
  public enum AuthorityFlags {
    Proxy          = 0,
    StateAuthority = 1,
    InputAuthority = 2,
  }
  ...
  public struct TRSThresholds {
    ...
    /// Only applicable when there is no <see cref="InterpolationTarget"/> set.
    /// </summary>
    [InlineHelp]
    public bool UseEnergy;
    /// <summary>
    /// The SqrMagnitude of the difference between the current position of the Rigidbody and interpolated position.
    /// If the SqrMagnitude of the difference is less than this value, then the Rigidbody will not be changed during Render,
    ...
    /// <summary>
    /// The SqrMagnitude of the difference between the current localScale of the Rigidbody and interpolated localScale.
    /// If the SqrMagnitude of the difference is less than this value, then the Rigidbody will not be changed during Render,
    /// allowing the Rigidbody to sleep, thus retaining cached friction states.
    /// A value of 0 indicates that this threshold should not be factored in to determining if interpolation occurs.
    /// </summary>

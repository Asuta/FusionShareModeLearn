namespace Fusion
{
  ...
  using UnityEngine;
  public class EnableRunnerLagCompensationGizmos : Behaviour
  {
    ...
    private void Awake()
    {
      ...
      if (_runner == null) Destroy(this);
    }

    private void OnDrawGizmos()
    {
      if (_runner == null || _runner.IsRunning == false) return;
        ...
        foreach (var colliderDrawInfo in snapshotDrawInfo) {
          ...
          switch (colliderDrawInfo.Type) {
            ...
            case HitboxTypes.Sphere:
              Gizmos.DrawWireSphere(colliderDrawInfo.Offset, colliderDrawInfo.SphereRadius);
              break;
            default:

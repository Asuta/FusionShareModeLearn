...
#region Assets/Photon/Fusion/Runtime/FusionTypes.Common.cs

// merged Types

#region FieldsMask.cs
...
namespace Fusion
{
  ...
  /// <summary>
  /// Base class for <see cref="FieldsMask{T}"/>.
  /// </summary>
  [Serializable]
  public abstract class FieldsMask {
    
    // These are used by drawer code, even though the appear unused.
    public Mask256 Mask;

    /// <summary>
    /// Constructor for <see cref="FieldsMask"/>.
    /// </summary>
    ...
    /// <summary>
    /// Implicitly convert <see cref="FieldsMask"/> to its long mask value.
    /// </summary>
  ...
  /// NOTE: A <see cref="FieldsMask{T}"/> attribute is required for proper rendering in the Inspector.
  /// </summary>
  [Serializable]
  public class FieldsMask<T> : FieldsMask {

    /// <summary>
    /// Constructor for <see cref="FieldsMask{T}"/>.
    /// </summary>
    public FieldsMask(Mask256 mask) : base(mask) { }
    
    public FieldsMask(long maskA, long maskB = 0, long maskC = 0, long maskD = 0) : base(maskA, maskB, maskC, maskD) { }
    
    /// <summary>
    /// Constructor for <see cref="FieldsMask{T}"/>.
    /// </summary>
    public FieldsMask() : base() { }
    
    public FieldsMask(Func<Mask256> getDefaultsDelegate) : base() {
      Mask = getDefaultsDelegate?.Invoke() ?? new Mask256();
    }
  }
}
...
namespace Fusion {
  ...
  public unsafe struct Mask256 : IEquatable<Mask256> {
...
#if FUSION_UNITY || UNITY_2021_3_OR_NEWER
     [UnityEngine.SerializeField]
#endif
    private fixed long values[4];
    ...
    public void SetBit(int bitIndex, bool set) {
      if (set) {
        if (bitIndex < 64)  { values[0] |= (long)1 <<  bitIndex;        return; }
        if (bitIndex < 128) { values[1] |= (long)1 << (bitIndex - 64);  return; }
        if (bitIndex < 192) { values[2] |= (long)1 << (bitIndex - 128); return; }
        if (bitIndex < 256) { values[3] |= (long)1 << (bitIndex - 192); return; }
      } else {
        if (bitIndex < 64)  { values[0] &= ~((long)1 << bitIndex);         return; }
        if (bitIndex < 128) { values[1] &= ~((long)1 << (bitIndex - 64));  return; }
        if (bitIndex < 192) { values[2] &= ~((long)1 << (bitIndex - 128)); return; }
    ...
    public bool GetBit(int bitIndex) {
      if (bitIndex < 64)  { return (values[0] & (long)1 <<  bitIndex)        != 0;}
      if (bitIndex < 128) { return (values[1] & (long)1 << (bitIndex - 64))  != 0;}
      if (bitIndex < 192) { return (values[2] & (long)1 << (bitIndex - 128)) != 0;}
      if (bitIndex < 256) { return (values[3] & (long)1 << (bitIndex - 192)) != 0;}

      return false;
    }

    public Mask256(long a, long b = 0, long c = 0, long d = 0) : this() {
      values[0] = a;
      values[1] = b;
      values[2] = c;
      values[3] = d;
    ...
    public static Mask256 operator ~(Mask256 a) {
      return new Mask256(
        ~a.values[0],
        ~a.values[1] ,
        ~a.values[2] ,
...
#region FusionScriptableObject.cs
...
#region FusionSerializableType.cs
...
namespace Fusion {
  ...
  [Serializable]
  public struct FusionSerializableType {
    ...
    public bool IsValid => !string.IsNullOrEmpty(_assemblyQualifiedName);
    
    public FusionSerializableType(Type type) {
    ...
    public Type Value {
      get {
        ...
        if (!_cache.TryGetValue(_assemblyQualifiedName, out var _value)) {
          try {
            _value = Type.GetType(_assemblyQualifiedName, true);
          } catch (Exception e) {
            _value = ExceptionDispatchInfo.Capture(e);
          }
          _cache.Add(_assemblyQualifiedName, _value);
        }
        
        Debug.Assert(_value != null);
        if (_value is ExceptionDispatchInfo exception) {
          exception.Throw();
        }
    ...
    public static implicit operator FusionSerializableType(Type type) {
      return new FusionSerializableType(type);
    }

    public static implicit operator Type(FusionSerializableType serializableType) {
...
namespace Fusion {
  ...
  public partial class FusionUnityLogger : Fusion.ILogger {
    ...
    public void LogException(Exception ex, in LogContext logContext) {
      Log(LogType.Error, $"{ex.GetType()}\n<i>See next error log entry for details.</i>", in logContext);
